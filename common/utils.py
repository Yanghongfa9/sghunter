...
import dill
from helper import create_pos_neg_dict
#needed for dill
from helper import hash as g_hash
...
#     else:
    ...
    # # 将 hash2graph 写入 JSON 文件
    # with open(f'{data_dir}{numberOfNeighK}hash2graph.json', 'w') as f:
    #     for key in hash2graph.keys():
    #         for graph_key in hash2graph[key].keys():
    #             hash2graph[key][graph_key] = nx.node_link_data(hash2graph[key][graph_key])
    #     json.dump(hash2graph, f)

    # # 将 posQueryHashes 写入 JSON 文件
    # with open(f'{data_dir}{numberOfNeighK}posQueryHashes.json', 'w') as f:
    #     for key in posQueryHashes.keys():
    #         for set_key in posQueryHashes[key].keys():
    #             posQueryHashes[key][set_key] = list(posQueryHashes[key][set_key])
    #     json.dump(posQueryHashes, f)
    #
    # # 将 posQueryHashStats 写入 JSON 文件
    # with open(f'{data_dir}{numberOfNeighK}posQueryHashStats.json', 'w') as f:
    #     json.dump(posQueryHashStats, f)

    # 将 hash2seed 写入 JSON 文件
    # with open(f'{data_dir}{numberOfNeighK}hash2seed.json', 'w') as f:
    #     for key in hash2seed.keys():
    #         hash2seed[key] = list(hash2seed[key])
    #     json.dump(hash2seed, f)

    print('data loaded')
    

def create_neg_query(graphs,org_node,isTrain,nodes_data,posQueryHashStats,
                     posQueryHashes=None,hash2graph=None,hash2seed=None,recursiveCount=0,
                     procCandidatesTest=None,procCandidatesTrain=None):
    global  negQueryHashes1
    if recursiveCount > 100:
        return None, None, None, None

    global nodes_data_train, nodes_data_test
    # save_file=f'/home/yhf/sghunter/data/ta1-theia-e3-official-6r/test_neg_dict_3.pc'
    # procCandidatesTest = pc.load(open(save_file,'rb'))
    # save_file=f'/home/yhf/sghunter/data/ta1-theia-e3-official-6r/train_neg_dict_3.pc'
    # procCandidatesTrain = pc.load(open(save_file,'rb'))
    if isTrain:
    ...
    if len(candidate_paths) > 0:
        ...
        #possible_seeds = list(set(hash2seed[neg_path_hashes]) & set(list(graphs.keys())))

        #neg_seed = random.choices(possible_seeds)[0]
        # print('neg_seed', len(possible_seeds))
        #assert len(possible_seeds) != 0
        neg_seed1 = random.choices(list(hash2seed[neg_path_hashes]))[0]
        neg_seedg = random.choices(list(graphs.keys()))[0]
        initial_graph = hash2graph[neg_seed1][neg_path_hashes].copy()
        #initial_graph = hash2graph[neg_seed][neg_path_hashes].copy()
        # neg_seed = list(initial_graph.nodes)[0]
        graph = graphs[neg_seedg]

        min_size=min(50,len(graph))
        max_size=min(100,len(graph))
        size = random.randint(min_size,max_size)

        ne = findNeighPathFromPath(graph,initial_graph, size)
        if len(ne)>max(min_size,1):
            return None, ne, 'new method', neg_seed1
...
def create_pos_query(graph, node, isTrain,nodes_data,posQueryHashStats,posQueryHashes=None,hash2graph=None,recursiveCount=0):
    global posQueryHashes1
    #posQueryHashStats = dill.load(open(f'/home/yhf/sghunter/data/ta1-theia-e3-official-6r/3posQueryHashStats.pkl', 'rb'))
    # if isTrain:
    dictKey = 'train' if isTrain else 'test'
    ...
    if len(candidate_paths) > 0:
        ...
        if sum(weights)==0:
            ...
            min_size=min(10,len(graph))
            max_size=min(50,len(graph))
            size = random.randint(min_size,max_size)
...
def findNeighPathFromPath_ifno(graph, initial_graph, size):
    while len(initial_graph) < size:
        # 1. 随机选择一个节点
        node = random.choice(list(initial_graph.nodes()))

        if node not in graph.nodes() or not list(graph.neighbors(node)):
            # 如果当前节点没有邻居，则从graph中随机选择一个大小为size的子图
            subgraph_nodes = random.sample(graph.nodes(), size)
            subgraph = graph.subgraph(subgraph_nodes)

            # 随机选择initial_graph中的一个节点，并将其与subgraph中的一个节点相连
            random_initial_node = random.choice(list(initial_graph.nodes()))
            random_subgraph_node = random.choice(list(subgraph.nodes()))
            initial_graph.add_edge(random_initial_node, random_subgraph_node)
            initial_graph = nx.compose(initial_graph, subgraph)
        else:
            neighbors = list(graph.neighbors(node))
            # 随机选择一个邻居节点
            neighbor = random.choice(neighbors)

            # 如果初始图中不存在选定的邻边，则将该邻边添加到初始图中
            if not initial_graph.has_edge(node, neighbor):
                initial_graph.add_edge(node, neighbor)
...
def findNeighPathFromPath(graph,initial_graph, size):
    ...
    if len(list(initial_graph.nodes)) < size:
        initial_graph = findNeighPathFromPath_ifno(graph, initial_graph, size)
    return initial_graph
