...
import pickle as pc
type2newType=pc.load(open('/home/yhf/sghunter/helper/eventTyperInt.pt','rb'))
import random

def load_train(file,feature_type,minLimit=5,maxlimit=1000):
...
class DiskDataSource(DataSource):
    ...
    def gen_batch_child(self, a, b, c, train, max_size=25, min_size=15, seed=None,nodes_data=None,posQueryHashStats=None,
        filter_negs=False, sample_method="tree-pair",posQueryHashes=None,hash2seed=None,hash2graph=None,procCandidatesTest=None,procCandidatesTrain=None):
        record = gen_batch_childMP(a,b, c, train,self.node_anchored,self.dataset, max_size, min_size, seed, nodes_data,posQueryHashStats,filter_negs, sample_method,
                                   posQueryHashes=posQueryHashes,hash2seed=hash2seed,hash2graph=hash2graph,procCandidatesTest=procCandidatesTest,procCandidatesTrain=procCandidatesTrain)
    ...
    def gen_batch(self, a, b, c, train,max_size=2000, min_size=100, seed=None,nodes_data=None,filter_negs=False, sample_method="tree-pair",posQueryHashStats=None,posQueryHashes=None,hash2seed=None,hash2graph=None,procCandidatesTest=None,procCandidatesTrain=None):
        
        batch_size = a
        self.initialazeArrays()
        self.gen_batch_child(a, b, c, train, max_size=max_size, min_size=min_size, seed=seed,nodes_data=nodes_data,
                             posQueryHashStats=posQueryHashStats,posQueryHashes=posQueryHashes,hash2seed=hash2seed,hash2graph=hash2graph,
                            filter_negs=filter_negs, sample_method=sample_method,procCandidatesTest=procCandidatesTest,procCandidatesTrain=procCandidatesTrain)
...
def gen_batch_childMP(a, b, c, train,node_anchored,dataset, max_size=25, min_size=15, seed=None,nodes_data=None,posQueryHashStats=None,
    filter_negs=False, sample_method="tree-pair",posQueryHashes=None,hash2seed=None,hash2graph=None,procCandidatesTest=None,procCandidatesTrain=None):
...
#         global dataset
    ...
    while len(pos_a) < (batch_size // 2):  
        ...
        dictKey = 'train' if train else 'test'
        node_a = a[0]
        for node in a:
            if node in nodes_data.index:
                node_path = nodes_data.loc[node].path
                if node_path in list(posQueryHashStats[dictKey].keys()) and node in list(posQueryHashes[node_path].keys()) and node_path!='/dev/urandom':
                    node_a = node
                    break
        try:
            _,b = utils.create_pos_query(graph, node_a, train,nodes_data,posQueryHashStats,posQueryHashes,hash2graph)
        except:
            continue
        ...
        for node in a:
            if node in nodes_data.index:
                ...
                if node_path in list(posQueryHashStats[dictKey].keys()) and node in list(posQueryHashes[node_path].keys()) and node_path!='/dev/urandom':
                    ...
                    break
        try:
            graph_b, b, neg_source, b_anchor = utils.create_neg_query(graphs,node_a,train,nodes_data=nodes_data,hash2seed=hash2seed,
                                                                      hash2graph=hash2graph,posQueryHashes=posQueryHashes,posQueryHashStats=posQueryHashStats,
