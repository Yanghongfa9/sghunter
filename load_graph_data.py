...
def replace_edge_attributes(nx_graph, event_index):
    ...
    return graph
def read_provenance_graph_and_convert_to_adjacency_matrix(filename,uuid_all,detected_nodes_indexes):
    path = '/home/yhf/sghunter/query_graph/graph/theia/node_prograph/'
...
def load_data(feature='ta1-theia-e3-official-6r',numberOfNeighK=3):
    ...
    def getType(row):
        tp = row['type']
        feature_dir = f'/home/yhf/sghunter/node_feature/ta1-theia-e3-official-6r/'
    ...
    return nodes_data,hash2graph,posQueryHashes,posQueryHashStats,hash2seed
...
def generate_sghunter_dataset():
    global nodes_data,hash2graph,posQueryHashes,posQueryHashStats
    posQueryHashes = defaultdict(lambda: defaultdict(set))
    posQueryHashStats = defaultdict(lambda: defaultdict(int))
    nodes_data,hash2graph,posQueryHashes,posQueryHashStats,hash2seed = load_data()
    # index_nodes_data = nodes_data.index.to_list()
    detections = read_txt_file('/home/yhf/sghunter/query_graph/graph/theia/detection.txt')
    # path_to_uuid = {}
    nodesdata_path_to_uuid={}
    uuid_all=[]
    # with open('/home/yhf/sghunter/node_feature/ta1-theia-e3-official-6r/all_nodes_data.csv', 'r') as csvfile:
    ...
    with open('/home/yhf/sghunter/node_feature/ta1-theia-e3-official-6r/nodes_data.csv', 'r') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            path = row['path']
            uuid_node = row['uuid']
            uuid_all.append(uuid_node)
            # if path !='':
            #     if path in nodesdata_path_to_uuid:
            #         nodesdata_path_to_uuid[path].append(uuid_node)
            #     else:
            #         nodesdata_path_to_uuid[path] = [uuid_node]
    train_dataset = {}
    test_dataset = {}
    ...
    for detection in detections:
        if count< 8:
            count += 1
            # if detection in path_to_uuid:
            # flag = False
            graph,detected_nodes_index = read_provenance_graph_and_convert_to_adjacency_matrix(detection,uuid_all,detected_nodes_indexes)
            detected_nodes_indexes.append(detected_nodes_index)
            # for graph_uuid in list(graph.nodes._nodes.keys()):
            #     if graph.nodes._nodes[graph_uuid]['data_node'].unidname in nodesdata_path_to_uuid:
            #         train_dataset.update({graph_uuid:graph})
            #         flag = True
            #         break
            train_dataset.update({detected_nodes_index:graph})
            # else:
            #     new_uid = str(uuid.uuid4()).upper()
            #     train_dataset.update({new_uid:read_provenance_graph_and_convert_to_adjacency_matrix(detection,path_to_uuid)})
            #     uuid_all.append(new_uid)
            # if not flag :
            #     train_dataset.update({detected_nodes_index:graph})
            #     with open('/home/yhf/sghunter/node_feature/ta1-theia-e3-official-6r/nodes_data.csv', 'a') as csvfile:
            #         writer = csv.writer(csvfile)
            #         writer.writerow([detected_nodes_index,detection,'network'])
        else:
            flag = False
            graph,detected_nodes_index = read_provenance_graph_and_convert_to_adjacency_matrix(detection,uuid_all,detected_nodes_indexes)
            detected_nodes_indexes.append(detected_nodes_index)
            ...
            # for graph_uuid in list(graph.nodes._nodes.keys()):
            #     if graph_uuid in index_nodes_data:
            #         test_dataset.update({graph_uuid:graph})
            #         flag = True
            #         break
            # if detection in path_to_uuid:
            #     test_dataset.update({path_to_uuid[detection]:read_provenance_graph_and_convert_to_adjacency_matrix(detection,path_to_uuid)})
            # else:
            #     new_uid = str(uuid.uuid4()).upper()
            #     test_dataset.update({new_uid:read_provenance_graph_and_convert_to_adjacency_matrix(detection,path_to_uuid)})
            #     uuid_all.append(new_uid)
            # if not flag:
            #     test_dataset.update({detected_nodes_index:graph})
            #     with open('/home/yhf/sghunter/node_feature/ta1-theia-e3-official-6r/nodes_data.csv', 'a') as csvfile:
    ...
    print('data----over')
    data_identifier ='ta1-theia-e3-official-6r'
    numberOfNeighK = 3
    node_anchored  = True
    ...
    for batch_target, batch_neg_target, batch_neg_query in zip(*loaders):
        batch_train = data_source.gen_batch(batch_target, batch_neg_target, batch_neg_query, True,nodes_data=nodes_data,
                                      posQueryHashStats=posQueryHashStats,hash2graph=hash2graph,posQueryHashes=posQueryHashes,hash2seed=hash2seed)
        # batch = [elem.to(utils.get_device()) for elem in batch_train]
    ...
    for batch_target, batch_neg_target, batch_neg_query in zip(*loaders):
        ...
        #batch_test = move_to_device(batch_test)
...
def test_layers(train_pts, test_pts, args, model, optimizer, criterion):
    val_acc = 0.0
    for epoch in range(1, 1 +100):
        ...
        with torch.no_grad():
            for data in test_pts:
                ...
                loss = criterion(outputs, labels.double())
                val_loss += loss.item()
                preds = (outputs > 0.58).long()
                total_correct += torch.sum(preds == labels).item()
                total_samples += 1
                # 保存预测结果和真实标签，用于后续计算指标
        val_acc = total_correct / total_samples  # 计算准确率
        print(epoch,training_acc/512,val_acc)
        # path = f"/home/yhf/sghunter/ckpt/{args.filename}-seed-05.pth"
        # torch.save(model.state_dict(), path)
    return val_acc

def layer_plot(n_layers, val_acc):
    # 使用plt.plot()函数绘制折线图
    plt.plot(n_layers, val_acc, marker='o', linestyle='-')
    ...
    # 显示图例
    plt.legend(['Average Accuracy'], loc='lower right')
    # 保存图片
    plt.savefig('test_layers.png')
    # 显示图形
    plt.grid(True)
    plt.show()
if __name__ == '__main__':
    ...
    # args.n_layers = [3,4,5,6,7,8,9,10]
    # layers =[3,4,5,6,7]
    args.n_layers = 4
    args.model_path = '/home/yhf/sghunter/ckpt/SeedGNN-model-ta1-theia-e3-official-6r-trained.pths'
    # args.filename ='theia_hidden_dim'
    args.filename = 'SeedGNN-model-ta1-theia-e3-official-6r-trained'
    val_acc = []
    # for layer in layers:
    # for hidden_dim in hidden_dims:
    #     args.model_path = f'/home/yhf/sghunter/ckpt/{args.filename}-{hidden_dims}.pth'
    ...
    #     print(f'hidden_dim {hidden_dim} is over')
    # # layer_plot(layers, val_acc)
    # print(val_acc)
    model = build_seedgnn(args).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    ...
    #
    # path = "/home/yhf/sghunter/ckpt/SeedGNN-model-ta1-theia-e3-official-6r-trained.pth"
    # torch.save(model.state_dict(), path)
    ...
    with torch.no_grad():
        for data in test_pts:
            ...
            # 保存预测结果和真实标签，用于后续计算指标
    # 计算平均每个图的时间
    average_time_per_graph = total_time / 512

    print(f'Average time per graph: {average_time_per_graph:.6f} seconds')