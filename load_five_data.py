...
def generate_subgraph(graph, num_nodes):
    selected_nodes = set()

    while len(selected_nodes) < num_nodes:
        # 选择节点的方法可以根据需求进行调整，这里以随机选择为例
    ...
    for edge in graph.edges(data=True):
        if edge[0] in selected_nodes and edge[1] in selected_nodes:
            subgraph.add_edge(edge[0], edge[1], data_edge=edge[2]['data_edge'])
...
def read_provenance_graph_and_convert_to_adjacency_matrix(filename,uuid_all,path_to_uuid,detected_nodes_indexes,path):
    # path2 =  '/home/yhf/sghunter/query_graph/graph/five/prograph/'
    nx_G = json2graph(path,filename)
    # nx_G_union = nx.MultiDiGraph()
    # while nx_G_union.number_of_nodes() < 200:
    #     num_nodes = random.randint(2000, 3000)
    #     num_nodes = min(num_nodes, nx_G.number_of_nodes())
    #     subgraphs = generate_subgraph(nx_G, num_nodes)
    ...
    #     nx_G_changeNode,detected_nodes_index = change_node_feature(subgraphs,path_to_uuid,detected_nodes=filename)
    #     if nx_G_union.number_of_nodes() == 0:
    #         nx_G_union = nx_G_changeNode
    #     else:
    #         nx_G_union = nx.compose(nx_G_union,nx_G_changeNode)
    # graph2json(path,nx_G_union, filename)
...
def load_data(feature='fiveDirection',numberOfNeighK=3):
    ...
    def getType(row):
        tp = row['type']
        feature_dir = f'/home/yhf/sghunter/node_feature/fiveDirection/'
...
def generate_sghunter_dataset():
    ...
    detections = read_txt_file('/home/yhf/sghunter/query_graph/graph/five/detection_five.txt')
    path_to_uuid = {}
    uuid_all=[]
    #
    # #抽象节点以及合并重复节点
    # with open('/home/yhf/sghunter/node_feature/fiveDirection/all_nodes_data.csv', 'r') as csvfile:
    ...
    #定位关键节点
    with open('/home/yhf/sghunter/node_feature/fiveDirection/nodes_data.csv', 'r') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            path = row['path']
            uuid_node = row['uuid']
    ...
    path = '/home/yhf/sghunter/query_graph/graph/five/node_prograph/'
    train_dataset = {}
    test_dataset = {}
...
if __name__ == '__main__':
    ...
    # hidden_dims = 16
    args.n_layers = 4
    args.model_path = '/home/yhf/sghunter/ckpt/SeedGNN-model-five-trained.pt5'
    args.filename = 'five'
    # layers = [3,4,5,6,7]
    # val_acc = []
    # # for layer in layers:
    ...
    # args.model_path = f'/home/yhf/sghunter/ckpt/five-4.pth'
    ...
    path = "/home/yhf/sghunter/ckpt/SeedGNN-model-five-trained.pth"
